...
import { executeMvpAutoAutonomous, executeMvpAutoDeepSearch, executeMvpAutoSearch, planMvpAuto } from "@/lib/mvpauto";
export const Route = createFileRoute("/api/mvpauto")({
  server: { handlers: {
    ...
    POST: async ({ request }) => {
      try {
        const body = await request.json() as { goal?: unknown; context?: unknown; preferredProvider?: unknown; execute?: unknown; deep?: unknown; autonomous?: unknown; deadlineMs?: unknown; start?: unknown };
        ...
        const result = body.autonomous === true ? await executeMvpAutoAutonomous({ ...input, deadlineMs: typeof body.deadlineMs === "number" ? body.deadlineMs : undefined }) : body.deep === true ? await executeMvpAutoDeepSearch(input) : body.execute === true ? await executeMvpAutoSearch(input) : planMvpAuto(input);
