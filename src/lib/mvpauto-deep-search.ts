...
async function json(url: string) {
  const r = await fetch(url, { headers: { accept: "application/json", "user-agent": "MVPAUTO/1.0" } });
...
export async function deepSearch(query: string) {
  ...
  const [ddg, github, npm] = await Promise.allSettled([
    fetch("https://html.duckduckgo.com/html/?q=" + q, {
      headers: { accept: "text/html", "user-agent": "Mozilla/5.0 MVPAUTO/1.0" },
    })
      .then(async (r) => (r.ok ? r.text() : ""))
      .then((html) =>
        [...html.matchAll(/<a[^>]+class=["'][^"']*result__a[^"']*["'][^>]+href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)]
          .slice(0, 5)
          .map((m) => ({ url: m[1], title: clean(m[2]), source: "duckduckgo" as const })),
      ),
    json("https://api.github.com/search/repositories?q=" + q + "&sort=stars&per_page=5").then((j) =>
      (j.items ?? []).map((i: any) => ({
        url: i.html_url,
        title: i.full_name,
    ...
    json("https://registry.npmjs.org/-/v1/search?text=" + q + "&size=5").then((j) =>
      (j.objects ?? [])
        .map((o: any) => ({
        ...
        .filter((x: DeepSource) => !!x.url),
    ),
  ]);

  const sources: DeepSource[] = [ddg, github, npm]
  ...
  const contents = await Promise.allSettled(
    sources.map(async (s) => {
      ...
      const response = await fetch(s.url, {
        headers: { accept: "text/html,text/plain,application/json", "user-agent": "MVPAUTO/1.0" },
      });
  ...
  return {
    query,
    sources: contents.flatMap((x) => (x.status === "fulfilled" ? [x.value] : [])),
